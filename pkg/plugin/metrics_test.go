...
// TestMetricsV2 tests /metrics handler.
func TestMetricsV2(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			key:        "test-key",
			encryptErr: errors.New("fail"),
			expects:    `aws_encryption_provider_kmsv2_operations_total{key_arn="test-key",operation="encrypt",status="failure"} 1`,
		},
		{
			key:        "test-key-throttle",
			encryptErr: awserr.New("RequestLimitExceeded", "test", errors.New("fail")),
			expects:    `aws_encryption_provider_kmsv2_operations_total{key_arn="test-key-throttle",operation="encrypt",status="failure-throttle"} 1`,
		},
	}
	for i, entry := range tt {
		t.Run(entry.key, func(t *testing.T) {
			...
			_, err := p.Encrypt(context.Background(), &pb2.EncryptRequest{Plaintext: []byte("hello")})
			if err != nil {
				if entry.encryptErr == nil {
					t.Fatal(err)
				}
