...
func main() {
	var (
		...
		kmsVersions   = flag.StringSlice("kms-versions", []string{"v1:v2"}, "comma-separated list of colon-separated kms version lists (e.g. 'v1:v2' or 'v1:v2,v2' if you have 2 keys)")
	...
	if len(*keys) != len(*addrs) || len(*kmsVersions) != len(*keys) {
		fmt.Fprintf(os.Stderr, "key, listen, and kms-versions lists must have the same number of elements")
		os.Exit(1)
	}
	...
	for i, key := range *keys {
		kms := (*kmsVersions)[i]
		kmsList := strings.Split(kms, ":")
		s := server.New()
		servers = append(servers, s)
		if (slices.Contains(kmsList, "v1")) {
			p := plugin.New(key, c, *encryptionCtx, sharedHealthCheck)
			p.Register(s.Server)
		}
		if (slices.Contains(kmsList, "v2")) {
